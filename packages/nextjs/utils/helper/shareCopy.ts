...
export const buildShareCopy = (options: ShareCopyOptions) => {
  ...
  if (context === "results" || resultsFinalized) {
    return `Proof-backed results for ${descriptor} "${prompt}" on Shadow (powered by @zama) are live. Peek at the decrypt-and-publish reveal.`;
  }

  if (resultsOpened && !resultsFinalized) {
    return `Encrypted tallies for ${descriptor} "${prompt}" are being revealed on Shadow (powered by @zama). Watch the proofs drop when they publish.`;
  }

  if (userHasVoted) {
    return `I just cast a private vote on Shadow (powered by @zama) for "${prompt}". Tallies stay sealed until ${deadlineText}${countdown ? ` (${countdown} left)` : ""}.`;
  }
  ...
  if (deadline * 1000 > Date.now()) {
    return `Spotted this ${descriptor} "${prompt}" on Shadow (powered by @zama). Votes stay encrypted until ${deadlineText}${countdown ? ` (${countdown} left)` : ""}. ${inviteLine}`;
  }

  return `Voting just closed on ${descriptor} "${prompt}" via Shadow (powered by @zama). Stick around for the encrypted tally reveal and on-chain publish.`;
};
